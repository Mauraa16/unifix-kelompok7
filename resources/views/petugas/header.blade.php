<nav class="bg-gradient-to-r from-purple-600 to-indigo-700 text-white shadow-md fixed w-full z-10" style="height: 70px;">
        ...
        <div class="flex justify-between items-center h-full">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    <form id="logout-form-petugas" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
                </div>
                @endauth
    ...
    {{-- MENU MOBILE PETUGAS --}}
    <div id="mobileMenu" 
         class="hidden md:hidden absolute top-[70px] left-0 w-full bg-purple-700 shadow-lg z-0 border-t border-purple-500">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <a href="{{ route('petugas.dashboard.index') }}" class="block px-3 py-2 rounded-md text-base font-medium hover:bg-purple-500 decoration-none text-white {{ request()->routeIs('petugas.dashboard.index') ? 'bg-purple-900' : '' }}">
                <i class="fas fa-tachometer-alt mr-2"></i> Dashboard
            </a>
            <a href="{{ route('petugas.laporan.index') }}" class="block px-3 py-2 rounded-md text-base font-medium hover:bg-purple-500 decoration-none text-white {{ request()->routeIs('petugas.laporan.*') ? 'bg-purple-900' : '' }}">
            ...
            <div class="border-t border-purple-500 pt-4 pb-2">
                ...
                <form id="logout-form-mobile-petugas" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
            </div>
            @endauth
        </div>
    </div>
