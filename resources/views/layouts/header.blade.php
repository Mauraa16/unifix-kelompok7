<nav x-data="{ open: false }" class="bg-gradient-to-r from-purple-600 to-indigo-700 text-white shadow-md fixed w-full z-50" style="height: 70px;">
    <div class="container mx-auto px-4 h-full">
        <div class="flex justify-between items-center h-full">
            ...
            <div class="flex items-center gap-4">
                @auth
                {{-- Ambil User dan Siapkan Cache Buster di awal auth block --}}
                @php 
                    $user = Auth::user(); 
                    ...
                    <a href="{{ route('mahasiswa.profil') }}" 
                       ...
                       title="Lihat Profil">
                        <div class="w-7 h-7 rounded-full flex items-center justify-center overflow-hidden">
                            ...
                            {{-- FOTO PROFIL DESKTOP (Cache Buster Diterapkan) --}}
                            @if ($user->foto_profil)
                                <img class="h-full w-full object-cover" 
                                     src="{{ asset('storage/' . $user->foto_profil) }}?v={{ $cacheBuster }}" 
                                     alt="Foto Profil">
                            @else
                                <div class="h-full w-full bg-white text-purple-700 flex items-center justify-center text-xs font-bold">
    ...
    <div x-show="open" 
        ...
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            @auth
            {{-- Ambil User dan Cache Buster lagi (meski sudah diambil di atas, agar aman) --}}
            @php 
                $user = Auth::user(); 
                $cacheBuster = \Carbon\Carbon::parse($user->updated_at)->timestamp;
            @endphp
            <div class="border-t border-purple-500 pt-4 mt-2 pb-2">
                <div class="flex items-center px-3 mb-3">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center border-2 border-purple-400 text-white font-bold overflow-hidden">
                        {{-- FOTO PROFIL MOBILE (Cache Buster Diterapkan) --}}
                        @if ($user->foto_profil)
                            <img class="h-full w-full object-cover" 
                                 src="{{ asset('storage/' . $user->foto_profil) }}?v={{ $cacheBuster }}" 
                                 alt="Foto Profil">
                        @else
                             <div class="h-full w-full bg-purple-800 flex items-center justify-center text-lg">
                                {{ strtoupper(substr($user->name, 0, 1)) }}
                            </div>
                        @endif
                    </div>
                    <div class="ml-3">
                        ...
                        <div class="text-sm font-medium text-purple-300">{{ $user->email }}</div>
                    </div>
                </div>
